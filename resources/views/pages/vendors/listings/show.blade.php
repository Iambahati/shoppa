<x-layouts.app>
    ...
    <div class="mb-6 flex items-center gap-3">
        <a href="{{ route('vendor.listings.index') }}" class="text-sm text-stone-400 hover:text-stone-600 transition-colors flex items-center gap-1">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="rounded-xl bg-white ring-1 ring-stone-950/5 shadow-sm p-6">
                <div class="flex items-start justify-between gap-4 mb-4">
                    <h2 class="text-lg font-semibold text-stone-900">{{ $product->name }}</h2>
            ...
            {{-- Device identifiers --}}
            <div class="rounded-xl bg-white ring-1 ring-stone-950/5 shadow-sm p-6">
                <h3 class="text-sm font-semibold text-stone-900 mb-4">Device identifiers</h3>
                <dl class="grid grid-cols-2 gap-4">
                    <div>
                        <dt class="text-xs text-stone-400 uppercase tracking-wide">IMEI</dt>
                        <dd class="mt-1 text-sm font-mono text-stone-900">{{ $product->imei ?? '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs text-stone-400 uppercase tracking-wide">Serial number</dt>
                        <dd class="mt-1 text-sm font-mono text-stone-900">{{ $product->serial_number ?? '—' }}</dd>
                    </div>
                    @if($product->battery_health)
                    <div>
                        <dt class="text-xs text-stone-400 uppercase tracking-wide">Battery health</dt>
                        <dd class="mt-1 text-sm font-medium {{ $product->battery_health >= 80 ? 'text-emerald-600' : 'text-amber-600' }}">
        ...
        <div class="space-y-4">
            ...
            <div class="rounded-xl bg-white ring-1 ring-stone-950/5 shadow-sm p-5">
                <h4 class="text-sm font-semibold text-stone-900 mb-4">Pricing &amp; stock</h4>
                <dl class="space-y-3">
                    <div>
                        <dt class="text-xs text-stone-400 uppercase tracking-wide">Price</dt>
                        <dd class="mt-1 text-xl font-semibold text-stone-900">{{ $product->formattedPrice() }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs text-stone-400 uppercase tracking-wide">Stock</dt>
                        <dd class="mt-1 text-sm text-stone-700">{{ $product->quantity }} unit(s)</dd>
                    </div>
                    <div>
                        <dt class="text-xs text-stone-400 uppercase tracking-wide">Category</dt>
                        <dd class="mt-1 text-sm text-stone-700">{{ $product->category?->name ?? '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs text-stone-400 uppercase tracking-wide">Listed</dt>
                        <dd class="mt-1 text-sm text-stone-700">{{ $product->created_at->format('d M Y') }}</dd>
                    </div>
                </dl>
            </div>
