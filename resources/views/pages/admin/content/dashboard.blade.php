<x-layouts.dashboard>
    ...
    <div class="mb-8 flex items-start justify-between gap-4">
        <div>
            ...
            <p class="mt-0.5 text-[13px] text-stone-500">Content Manager overview</p>
        </div>
    ...
    <div class="mb-8 card px-6 py-5">
        <h3 class="mb-5 text-sm font-semibold text-stone-900">Publication funnel</h3>
        @php
            $funnelStages = [
                ['label' => 'Submitted for review', 'count' => $funnel['submitted'], 'color' => 'bg-blue-400',    'text' => 'text-blue-700'],
                ['label' => 'In review',             'count' => $funnel['in_review'], 'color' => 'bg-amber-400',   'text' => 'text-amber-700'],
                ['label' => 'Published &amp; live',  'count' => $funnel['approved'],  'color' => 'bg-emerald-500', 'text' => 'text-emerald-700'],
                ['label' => 'Rejected',              'count' => $funnel['rejected'],  'color' => 'bg-red-400',     'text' => 'text-red-700'],
            ];
            ...
            @foreach($funnelStages as $stage)
                <div>
                    <div class="mb-1.5 flex items-center justify-between text-xs">
                        ...
                        <span class="tabular-nums text-stone-600">{{ number_format($stage['count']) }}</span>
                    </div>
    ...
    <div class="card overflow-hidden">
        <div class="flex items-center justify-between border-b border-stone-100 px-6 py-4">
            <h3 class="text-sm font-semibold text-stone-900">Recently submitted products</h3>
            <a href="{{ route('admin.products.index') }}" class="text-xs font-medium text-emerald-600 transition-colors hover:text-emerald-700">View all</a>
        </div>
        @if($recentProducts->isEmpty())
            <div class="flex flex-col items-center justify-center px-6 py-16 text-center">
                <svg class="h-16 w-16 text-stone-200" viewBox="0 0 64 64" fill="none" aria-hidden="true">
                    ...
                    <circle cx="32" cy="32" r="8" fill="white" stroke="currentColor" stroke-width="1.5"/>
                ...
                <p class="mt-4 text-sm font-semibold text-stone-600">No products submitted yet</p>
                <p class="mt-1 text-xs text-stone-400">Products will appear here as vendors submit them for review.</p>
            </div>
        @else
            <ul role="list" class="divide-y divide-stone-100">
                @foreach($recentProducts as $product)
                    <li class="flex items-center gap-4 px-6 py-3.5 transition-colors hover:bg-stone-50">
                        ...
                        <a href="{{ route('admin.products.index') }}" class="shrink-0 text-xs font-medium text-emerald-600 hover:text-emerald-700">Review →</a>
                    </li>
                @endforeach
            </ul>
        @endif
