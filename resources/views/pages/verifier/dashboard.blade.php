<x-layouts.dashboard>
    ...
    <div class="mb-8 flex items-start justify-between gap-4">
        <div>
            ...
            <p class="mt-0.5 text-[13px] text-stone-500">Inspection lab overview</p>
        </div>
    ...
    {{-- ── TRUST ENGINE NOTICE ──────────────────────────────────────────── --}}
    <div class="mb-8 flex items-center gap-3 rounded-lg border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm text-blue-800">
        <span class="h-1.5 w-1.5 shrink-0 rounded-full bg-blue-500"></span>
        <p>Only the Verifier role can issue Trust Certificates — each carries a QR-backed UUID</p>
        <a href="{{ route('verifier.queue') }}" class="ml-auto shrink-0 text-xs font-semibold text-blue-700 transition-colors hover:text-blue-900">
            Open queue →
        </a>
    </div>
    ...
    <div class="mb-8 card overflow-hidden">
        <div class="border-b border-stone-100 px-6 py-4">
            <h3 class="text-sm font-semibold text-stone-900">Today's certification progress</h3>
            <p class="mt-0.5 text-xs text-stone-400">{{ $done }} of {{ $totalRing }} devices processed</p>
        </div>
        @if($totalRing > 0)
                ...
                <div class="shrink-0">
                    <svg viewBox="0 0 80 80" class="h-24 w-24" aria-hidden="true">
                        <circle cx="40" cy="40" r="36" fill="none" stroke="currentColor"
                                stroke-width="8" class="text-stone-100" />
                        @if($certDash > 0)
                            <circle cx="40" cy="40" r="36" fill="none" stroke="currentColor"
                                    stroke-width="8" class="text-emerald-500"
                        ...
                        <text x="40" y="37" text-anchor="middle" font-size="15" font-weight="700"
                              fill="#0c0a09" font-family="Inter, sans-serif">{{ $certificationRing['certified'] }}</text>
                        <text x="40" y="52" text-anchor="middle" font-size="7" fill="#78716c"
                              font-family="Inter, sans-serif">certified</text>
                    </svg>
                </div>
                <div class="flex-1 space-y-3">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm text-stone-600">Certified</span>
                        </div>
                        <span class="text-sm font-semibold tabular-nums text-stone-900">{{ $certificationRing['certified'] }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <div class="h-2.5 w-2.5 rounded-full bg-red-400"></div>
                            <span class="text-sm text-stone-600">Rejected</span>
                        </div>
                        <span class="text-sm font-semibold tabular-nums text-stone-900">{{ $certificationRing['rejected'] }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <div class="h-2.5 w-2.5 rounded-full bg-stone-300"></div>
                            <span class="text-sm text-stone-600">Remaining</span>
                        </div>
                        <span class="text-sm font-semibold tabular-nums text-stone-900">{{ $certificationRing['remaining'] }}</span>
                    </div>
                    <div class="border-t border-stone-100 pt-3">
                        <div class="mb-1.5 flex items-center justify-between text-xs">
                            <span class="text-stone-500">Completion rate</span>
                            <span class="font-semibold text-stone-900">{{ $completionPct }}%</span>
                        </div>
        ...
        @else
            <div class="flex flex-col items-center justify-center px-6 py-10 text-center">
                <svg class="h-14 w-14 text-stone-200" viewBox="0 0 56 56" fill="none" aria-hidden="true">
                ...
                <p class="mt-3 text-sm font-medium text-stone-600">No activity today yet</p>
                <p class="mt-1 text-xs text-stone-400">Certifications will appear here as you process devices</p>
            </div>
        @endif
    </div>
    ...
    <div class="card overflow-hidden">
        <div class="flex items-center justify-between border-b border-stone-100 px-6 py-4">
            <h3 class="text-sm font-semibold text-stone-900">Oldest pending devices</h3>
            <a href="{{ route('verifier.queue') }}" class="text-xs font-medium text-emerald-600 transition-colors hover:text-emerald-700">Full queue ({{ $stats['queue_depth'] }})</a>
        </div>
        ...
        @if($topQueue->isEmpty())
            <div class="flex flex-col items-center justify-center px-6 py-16 text-center">
                ...
                <p class="mt-4 text-sm font-semibold text-stone-600">Queue is clear</p>
                <p class="mt-1 text-xs text-stone-400">All submitted devices have been processed.</p>
            </div>
        @else
            <ul role="list" class="divide-y divide-stone-100">
                @foreach($topQueue as $item)
                    @php
                        $device = $item['model'];
                        $urgencyColor = match($item['urgency']) {
                            'high'   => 'red',
                            'medium' => 'amber',
                            default  => 'stone',
                        };
                    ...
                    <li class="flex items-center gap-4 px-6 py-3.5 transition-colors hover:bg-stone-50">
                        ...
                        <x-ui-badge :color="$urgencyColor" size="xs">{{ $item['wait'] }}</x-ui-badge>
                        <a href="{{ route('verifier.queue') }}" class="shrink-0 text-xs font-medium text-emerald-600 transition-colors hover:text-emerald-700">
                            Inspect →
                        </a>
                    </li>
                @endforeach
